...
The newsletter previously covered fast-moving news and long research papers, with nothing in between for the slower, more considered writing that outlets and podcasts publish — so a curated list of blogs and podcasts can now be configured by name and feed address, and anything they published in the past week gets pulled in, summarized with its own dedicated set of instructions, and printed in a new section between News and Research Papers. Each source must be given as a direct RSS/Atom feed address rather than just any web page — trying to guess a feed from an arbitrary page, or scrape one that has none, turned out to be unreliable and was dropped in favour of only ever using known-good feeds, hand-verified when added. Podcasts work the same way, summarized from what their feed provides as show notes — several of the podcasts on the initial wish list turned out to have no public feed at all (their listening pages on Spotify do not expose one), so those are left out for now until a working feed is found for them. Testing also surfaced short, low-substance posts (an out-of-office notice, a newsletter sign-up teaser) that produced a summary describing the model's own confusion rather than being left out — these now drop cleanly instead of appearing in the newsletter. Each entry is written as a single 30-word-or-fewer sentence rather than a full paragraph, so the section reads as a scannable digest of what each outlet published rather than another block of full-length summaries.
